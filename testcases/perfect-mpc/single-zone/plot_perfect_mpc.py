...
measurement_base = pd.DataFrame(
    measurement_base, index=measurement_base['time'])
measurement_mpc = pd.DataFrame(measurement_mpc, index=measurement_mpc['time'])

tim_intp = np.arange(ts, te+1, 60)
measurement_base_60 = interpolate_dataframe(
    measurement_base[['PTot', 'TRoo']], tim_intp)
measurement_mpc_60 = interpolate_dataframe(
    measurement_mpc[['PTot', 'TRoo']], tim_intp)
measurement_base_900 = measurement_base_60.groupby(
    measurement_base_60.index//900).mean()  # every 15 minutes
measurement_mpc_900 = measurement_mpc_60.groupby(
    measurement_mpc_60.index//900).mean()  # every 15 minutes
...
#### get metrics
#================================================================================
metrics_base = get_metrics(
    measurement_base_900['PTot'].values, measurement_base_900['TRoo'].values.reshape(-1, 1), price_tou)
metrics_mpc = get_metrics(
    measurement_mpc_900['PTot'].values, measurement_mpc_900['TRoo'].values.reshape(-1, 1), price_tou)

metrics_base = pd.DataFrame(
    metrics_base, columns=[['energy', 'ene_cost', 'temp_violation']])
metrics_mpc = pd.DataFrame(
    metrics_mpc, columns=[['energy', 'ene_cost', 'temp_violation']])
