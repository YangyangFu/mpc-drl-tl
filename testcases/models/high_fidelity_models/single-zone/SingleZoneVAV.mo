...
package SingleZoneVAV
  ...
  model Airflow
    ...
    Modelica.Blocks.Math.MultiSum EHVAC(nu=2)  "Total HVAC energy"
...
First implementation.
  ...
  equation
    ...
    connect(con.yOutAirFra, hvac.uEco) annotation (Line(points={{-79,3},{-50,3},{
            -50,-2},{-42,-2}},             color={0,0,127}));
    connect(con.TMix, hvac.TMix) annotation (Line(points={{-102,0},{-112,0},{
            -112,-40},{10,-40},{10,-4},{1,-4}},             color={0,0,127}));
    ...
    connect(con.TOut, weaBus.TDryBul) annotation (Line(points={{-102,-3},{-108,
            -3},{-108,130}},              color={0,0,127}));
    connect(hvac.weaBus, weaBus) annotation (Line(
        points={{-36,17.8},{-36,130},{-108,130}},
        color={255,204,51},
        thickness=0.5));
    ...
    connect(TSetRooHea.y[1], oveTSetRooHea.u)
      annotation (Line(points={{-159,30},{-142,30}}, color={0,0,127}));
    connect(oveTSetRooHea.y, con.TSetRooHea) annotation (Line(points={{-119,30},
            {-116,30},{-116,6},{-102,6}},   color={0,0,127}));
    connect(TSetRooCoo.y[1], oveTSetRooCoo.u)
      annotation (Line(points={{-159,-10},{-142,-10}}, color={0,0,127}));
    connect(oveTSetRooCoo.y, con.TSetRooCoo) annotation (Line(points={{-119,-10},
    ...
    connect(weaBus.HGloHor, GHI) annotation (Line(
        ...
        extent={{-6,3},{-6,3}},
        horizontalAlignment=TextAlignment.Right));
    connect(con.yFan, hvac.uFan) annotation (Line(points={{-79,9},{-70,9},{-70,
            18},{-42,18}}, color={0,0,127}));
  ...
  package BaseClasses "Base classes for test case"
    ...
    model AirflowBase
      ...
      final parameter Modelica.SIunits.MassFlowRate mChiEva_flow_nominal=
        -QCoo_flow_nominal/Buildings.Utilities.Psychrometrics.Constants.cpWatLiq/6
    ...
    equation
      ...
      connect(out.ports[1], ideEco.port_1) annotation (Line(points={{-120,42},{-120,
      ...
      connect(senTRetAir.port_b, out.ports[2]) annotation (Line(points={{-40,-40},{-112,
              -40},{-112,36},{-120,36},{-120,38}},           color={0,127,255}));
      connect(TRet, senTRetAir.T) annotation (Line(points={{210,-100},{174,-100},{
              174,-20},{-30,-20},{-30,-29}}, color={0,0,127}));
      connect(gaiCooCoi.y,souCoo. m_flow_in) annotation (Line(points={{2,-120},{102,
              -120},{102,-102}},     color={0,0,127}));
      connect(uCooVal, gaiCooCoi.u) annotation (Line(points={{-220,10},{-160,10},{-160,
              -120},{-22,-120}}, color={0,0,127}));
      connect(souCoo.ports[1], cooCoi.port_a1) annotation (Line(points={{110,-80},{120,
              -80},{120,28},{110,28}}, color={0,127,255}));
      connect(cooCoi.port_b1, watRes.port_a)
        annotation (Line(points={{90,28},{72,28},{72,-50}}, color={0,127,255}));
      connect(watRes.port_b, sinCoo.ports[1])
        annotation (Line(points={{72,-70},{72,-82}}, color={0,127,255}));
