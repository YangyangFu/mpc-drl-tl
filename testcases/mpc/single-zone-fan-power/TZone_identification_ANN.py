...
ens = ensemble.GradientBoostingRegressor(**params)

# Third, create steps
steps_gbr = [('normalize',scaler),('reg',ens)]

# Foruth, create pipeline for evaluation
pipe_gbr = Pipeline(steps_gbr)

# Fifth, perform model selection using grid search to find the best trained ANN model
estimator_gbr = GridSearchCV(pipe_gbr,
...
t0 = time.time()
y_gbr = estimator_gbr.predict(X_test)
gbr_predict = time.time() - t0
print("ANN prediction for %d inputs in %.3f s"
      % (X_test.shape[0], gbr_predict))
...
relErr_gbr = (y_test-y_gbr)/y_test
print r2_gbr
...
plt.plot(y_gbr-273.15,'g-.',lw=0.5,markevery=0.05,marker='s',markersize=2,label='GBR')
plt.ylabel('Temperature (C)')
plt.legend()

plt.subplot(212)
plt.plot(y_ann-y_test, 'r-', lw=0.5, label='ANN')
plt.plot(y_gbr-y_test, 'g-', lw=0.5, label='GBR')
plt.ylabel('Error (C)')
plt.legend()
...
joblib.dump(estimator_gbr,'gbr.pkl')
