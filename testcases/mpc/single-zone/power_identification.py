...
l = 4
P_his = pd.DataFrame(data['P_total'])
for i in range(l):
    P_his['P_total_'+str(i+1)] = data['P_total'].values
    shift = P_his['P_total_'+str(i+1)].shift(periods=i+1)
    P_his['P_total_'+str(i+1)]=shift.values
P_his=P_his.drop(columns=['P_total'])
...
x_train = data_train[['P_total_1','P_total_2','P_total_3','P_total_4','mass_flow','T_oa']].values
y_train = data_train['P_total'].values

popt,pcov = curve_fit(func_P,x_train,y_train)
ypred_train = func_P(x_train,*popt)

# test on testing data
x_test = data_test[['P_total_1','P_total_2','P_total_3','P_total_4','mass_flow','T_oa']].values
y_test = data_test['P_total'].values
