...
low = 0. #action space lower bound
...
u_opt=273.15+12+6*actions[:,0]
v_opt=25+(410-25)*actions[:,1]
...
def rw_func(cost, penalty):
    ...
    res = (penalty * 1000 + cost*500) / 1000
    return res
...
rewards_ppo_v0_hist = rewards_ppo_v0_hist.loc[:(nepochs-1), 'Value']
rewards_ppo_v0 = [] 

for epoch in range(nepochs):
    rewards_ppo_v0 += [rewards_ppo_v0_hist[epoch]]
rewards_ppo_v0 = pd.DataFrame(np.array(rewards_ppo_v0),columns=['rewards'])
print (rewards_ppo_v0)
...
plt.plot(rewards_ppo_v0.values,'r--',lw=1,label='ppo')
