...
class FiveZoneAirEnv(object):
    ...
    Observation:
        ...
        0      Time                                          0              86400
        1      South Zone temperature                        273.15 + 12    273.15 + 30
        2      East Zone temperature                         273.15 + 12    273.15 + 30
        3      North Zone temperature                        273.15 + 12    273.15 + 30
        4      West Zone temperature                         273.15 + 12    273.15 + 30
        5      Core Zone temperature                         273.15 + 12    273.15 + 30
        6      Outdoor temperature                           273.15 + 0     273.15 + 40
        7      Solar radiation                               0              1200
        8      Total electricity power                       0              50000
        9      Maximum damper position of five zones         0              1
        10     Minimum damper position of five zones         0              1
    ...
    def _get_observation_space(self):
        ...
        1-5. zone temperatures for five zones; in K
        6. outdoor temperature; in K 
        7. solar radiation
        8. total power
        9. Maximum damper position of five zones
        10. Minimum damper position of five zones
        ...
        high = np.array([86400.] + [273.15+30]*5 +[273.15+40, 1200., 50000., 1., 1.])
        low = np.array([0.]+[273.15+12]*5 + [273.15+0,0, 0, 0, 0])
    ...
    def _reward_policy(self):
        ...
        power = states[8] 
        time = states[0]
        TZonSou = states[1] - 273.15 # orginal units from Modelica are SI units
        TZonEas = states[2] - 273.15
        TZonNor = states[3] - 273.15
        TZonWes = states[4] - 273.15
        TZonCor = states[5] - 273.15
        
        # Here is how the reward should be calculated based on observations
        
        num_zone = 5
        ZTemperature = [TZonSou,TZonEas,TZonNor,TZonWes,TZonCor] #temperature in C for each zone
        ...
        T_upper[7:19] = [24.5]*12 # upper bound for occuppied: cooling 
        T_lower[7:19] = [23.5]*12 # lower bound for occuppied: heating
...
class JModelicaCSFiveZoneAirEnv(FiveZoneAirEnv, FMI2CSEnv):
    """
    ...
    def __init__(self,
        ...
        config = {
            ...
            'model_output_names': ['time','TRooAirSou','TRooAirEas','TRooAirNor','TRooAirWes','TRooAirCor','TAirOut','GHI','PHVAC','yDamMax','yDamMin'],
            'model_parameters': {},
            'time_step': time_step,
        ...
        super(JModelicaCSFiveZoneAirEnv,self).__init__("./FiveZoneAir.fmu",
