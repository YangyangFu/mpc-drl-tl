...
class QRDQN(nn.Module):
    def __init__(self, state_shape, action_shape, num_quantiles, nlayers, device):
        super().__init__()
        ...
        sequences.append(nn.Linear(256, self.action_num * num_quantiles))
...
def test_qrdqn(args):
    ...
    net = QRDQN(args.state_shape, args.action_shape, args.num_quantiles, args.n_hidden_layers, args.device).to(args.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-quantiles', type=int, default=400)  
    ...
    # save results
    dt = {'qrdqn': te - ts}
    with open('qrdqn_time.json', 'w') as f:
        json.dump(dt, f)